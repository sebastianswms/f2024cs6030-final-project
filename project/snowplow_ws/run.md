...
I'm using ros2 and carla to control a vehicle. Below is the current code for my pure pursuit control node. It works well, but doens't handle all the functionality I want it to.

Specifically, right now it is effective at following waypoints and also at coming to a halt when a camera or lidar/radar obstacle is detected. However, my goal is to implement functionality that allows it to traverse backward.

To do this, a modified csv format will be provided. The waypoints.csv file contain multiple "batches" of waypoints. Each batch will be its own fully complete csv, and batches will be separated from one-another with lines containing only `===`. An extremely short example is shown below, just to demonstrate the format.
...
Each batch represents switching from forward to reverse gear. The goal is to start in forward and continue following waypoints until we reach the end of a batch. At this point, we come to a stop and reverse gears. Then we follow waypoints until we reach the end of a batch, switch to forward gear again, and continue. Repeat as many times as necessary until all batches have been exhausted.

You will likely need to reimplement `WaypointManager` entirely for this task, and may need to make significant modifications to `odometry_callback`.

A few additional things to keep in mind while coding:
 - Be mindful of off-by-one errors, especially when indexing into a list, series, or dataframe so we don't go out of bounds.
 - Take into account that when travelling backward, steering/yaw/etc. might need to be reversed to work appropriately.
